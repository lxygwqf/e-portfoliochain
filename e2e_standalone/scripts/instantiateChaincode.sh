...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.wisedu.com:7050 -C $CHANNEL_NAME -n idchaincode -v 1.0 -c '{"Args":["init"]}' -P "OR	('Org1MSP.peer','Org2MSP.peer','Org3MSP.peer')" >&log.txt
	else
